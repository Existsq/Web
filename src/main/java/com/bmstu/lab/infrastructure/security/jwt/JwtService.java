...
public class JwtService {
  ...
  public List<SimpleGrantedAuthority> extractRoles(JwtToken token) {
  ...
  public UserDetails extractUserDetails(JwtToken token) {
    ...
    List<SimpleGrantedAuthority> roleNames = extractRoles(token);

    log.trace("Extracted user '{}' with roles {}", username, roleNames);
    boolean isModerator = false;

    for (SimpleGrantedAuthority authority : roleNames) {
    ...
    return new User(username, isModerator);
  }
}
